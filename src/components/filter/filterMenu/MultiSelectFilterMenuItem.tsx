...
const MultiSelectFilterMenuItem = ({ column, values }: { column: any, values: string[] }) => {
    ...
    const onChange = (value: string) => {
        const updatedValues = selectedValues.includes(value) ? selectedValues.filter((item) => item !== value) : [...selectedValues, value];
            ...
            <div className='flex text-GRAY_600 items-center gap-[2px] w-full z-80 px-2.5'>
                ...
                <div className='hidden items-center gap-[2px] cursor-pointer relative select-none grow' onClick={() => setIsOpen(!isOpen)}>
                    ...
                    <SvgSpriteLoader
                        ...
                        width={12}
                    />
                    {isOpen && <div ref={ref} className=' p-1 z-10 absolute top-full left-0 w-[256px] bg-white text-GRAY_900 border border-GRAY_400 shadow-tableFilterMenu rounded-md'>
                        {
                            MULTI_SELECT_FILTER_OPTIONS.map((option) => (
                                <div className='hover:bg-GRAY_100 f-12-500 py-2 px-2.5 rounded-md' key={option.value} onClick={() => setSelectedOption(option)}>{option.label}</div>
                            ))
                        }
                    </div>}
                </div>
