...
import { FileHistoryDataType, HistoryListPropsType } from 'modules/data/components/importDataset/importData.types';
import { formattedDate, formattedFilename } from 'modules/data/components/importDataset/importData.utils';
...
const HistoryList: FC<HistoryListPropsType> = ({ isHoveredLoaders }) => {
  const data: FileHistoryDataType[] = [];
                      ...
                      <div className='flex justify-between items-center w-full'>
                        <div className='flex justify-start py-1.5 px-2 bg-GRAY_100 rounded-md w-fit'>
                          ...
                          <span className='f-12-400 text-GRAY_1000 ml-1.5'>
                            {formattedFilename(historyItem?.file_name)}
                          </span>
                        </div>
                        <StatusIndicator status={historyItem?.status as LOADER_STATUS} />
                      </div>
                      <span className='f-10-400 text-GRAY_700 mt-1'>{`${formattedDate(historyItem?.created_at)} by ${historyItem?.created_by}`}</span>
                    </div>
                  ))}
              </CommonWrapper>
            </div>
