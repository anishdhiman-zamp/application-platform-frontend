...
const SharePagePopup: FC<SharePagePopupPropsType> = ({ isOpen, onClose, pageId }) => {
  ...
  const handleShareDatasetPopup = async () => {
    try {
      await postInviteAudiences({ pageId, body: AudiencesDatasetShareData }).unwrap();
        ...
        {userAccessToDatasetList?.length > 0 && (
          <div className='bg-white mt-2 rounded-3.5 p-2 border border-GRAY_400'>
            ...
            <div className='flex flex-col w-full mt-2 max-h-[200px] overflow-y-scroll'>
              {userAccessToDatasetList?.map((audience, index) => <PageAccessToAudiences key={index} {...audience} />)}
            </div>
          </div>
        )}
      </Popup>
