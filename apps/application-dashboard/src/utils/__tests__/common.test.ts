...
describe('formatExpectedDuration', () => {
  ...
  it('returns "less than 3 minutes" for 120 seconds (exact multiple)', () => {
    expect(formatExpectedDuration(120)).toBe('less than 3 minutes');
  });

  it('returns "less than 4 minutes" for 180 seconds (exact multiple)', () => {
    expect(formatExpectedDuration(180)).toBe('less than 4 minutes');
  });
