...
const DynamicTabContextMenu = forwardRef<HTMLDivElement, DynamicTabContextMenuProps>(
  ({ children, tabIndex, totalTabs, disabled, onOpenChange, onActionClick }, ref) => {
    ...
    const getFilteredActions = (): TabContextMenuAction[] => {
      return TAB_CONTEXT_MENU_ACTIONS.filter((action) => {
        ...
        if (action.id === TAB_CONTEXT_MENU_ACTION_IDS.CLOSE_ALL && totalTabs <= 1) {
          return false;
        }
    ...
    return (
      <ContextMenu onOpenChange={handleOpenChange}>
        <ContextMenuTrigger asChild disabled={disabled} ref={ref}>
          {children}
        </ContextMenuTrigger>
        {isOpen && !disabled && (
...
DynamicTabContextMenu.displayName = 'DynamicTabContextMenu';
