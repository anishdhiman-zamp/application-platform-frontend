...
const MultiSelectFilter: FC<MultiSelectFilterProps> = ({
  ...
  return (
    <div
      className={cn(
        'border-0.5 border-GRAY_500 shadow-table-filter-menu flex max-h-[330px] w-[218px] min-w-[230px] flex-col gap-2 rounded-md bg-white pb-1 pt-2',
        className,
      )}
    >
      <div className='text-GRAY_600 z-80 flex w-full items-center gap-1 px-2.5'>
        ...
        <div
          className='relative flex grow cursor-pointer select-none items-center gap-[2px]'
        ...
        >
          ...
          />
          {isConditionOpen && (
            <div className='text-GRAY_900 border-GRAY_400 shadow-table-filter-menu absolute left-0 top-full z-10 rounded-md border bg-white p-1'>
              {operatorOptions.map((option) => (
                <div
                  className='hover:bg-GRAY_100 f-12-500 whitespace-nowrap rounded-md px-2.5 py-2'
                  key={option.value}
      ...
      {showSelectAll && (
        <div
          ...
          className='hover:bg-GRAY_100 mx-1 flex cursor-pointer select-none items-center justify-between gap-2 rounded px-2.5 py-2'
        >
      ...
      <div
        className={cn(
          'flex h-full flex-col overflow-y-auto overflow-x-hidden px-1 [&::-webkit-scrollbar]:hidden',
        ...
        {!!filteredValues?.length &&
          filteredValues.map((item) => (
            <div
              ...
              className='hover:bg-GRAY_100 flex cursor-pointer select-none items-center justify-between gap-2 rounded px-2.5 py-2'
            >
              {LabelComponent ? (
                LabelComponent(item)
              ) : (
