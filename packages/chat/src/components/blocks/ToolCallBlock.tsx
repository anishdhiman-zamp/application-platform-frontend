...
import { BrowserLiveWatchSection } from './BrowserLiveWatchSection';
...
export const ToolCallBlock: FC<ToolCallBlockProps> = ({
  ...
  const [isStreamingOpen, setIsStreamingOpen] = useState<boolean>(false);
  ...
  const screenshotUrl = useMemo(() => {
    if (!toolResult?.payload?.content) return undefined;
    const parsed = safeJsonParse<{
      screenshot_url?: string;
      live_url?: string;
      url?: string;
    }>(toolResult.payload.content);
    return parsed?.screenshot_url || parsed?.live_url || parsed?.url || undefined;
  }, [toolResult]);
  ...
  return (
    <div className='w-full'>
            ...
            <div className='flex flex-1 items-center gap-3'>
              ...
              {isBrowserTool && (
                <button
                  ...
                  className={cn(
                    'ml-auto flex items-center gap-1 rounded-full px-1.5 py-1 transition-colors',
                    isStreamingOpen ? 'bg-GRAY_100 text-GRAY_1000' : 'text-GRAY_900 hover:bg-GRAY_50',
                  )}
                >
                  {isStreamingOpen ? (
      ...
      {isBrowserTool && conversationId ? (
        <BrowserLiveWatchSection conversationId={conversationId} toolName={name} />
      ) : (
        isBrowserTool && (
          <LiveStreamingView
