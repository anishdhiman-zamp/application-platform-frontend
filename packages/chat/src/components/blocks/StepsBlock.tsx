...
const StreamingThinkingContent: FC<{ text: string; isStreaming: boolean }> = ({ text, isStreaming }) => {
  ...
  return <div className='f-12-400 text-GRAY_1000 whitespace-pre-wrap'>{displayedText}</div>;
};
...
export const StepsBlock: FC<StepsBlockProps> = ({ blocks, toolResultsMap, isStreaming = false }) => {
  ...
  return (
    <div className='border-border bg-BG_WHITE w-full overflow-hidden rounded-lg border'>
        ...
        {blocks.map((block, index) => {
          ...
          return (
            <AccordionItem key={`step-${index}`} value={`step-${index}`} className='relative border-none'>
              {!isFirst && (
                <div
                  className='bg-border pointer-events-none absolute top-0 left-[19.5px] h-[6px] w-px'
                  style={{ zIndex: 0 }}
                />
              )}
              <AccordionTrigger className='f-12-450 text-GRAY_1000 hover:bg-accent w-full cursor-pointer gap-x-2 py-2 pr-2 pl-3 hover:no-underline [&[data-state=closed]>svg]:rotate-90 [&[data-state=open]>svg]:-rotate-90'>
              ...
              <AccordionContent className='bg-muted max-h-60 overflow-y-auto py-2 pr-2 pl-11 [scrollbar-width:thin]'>
                {renderStepContent(block)}
              </AccordionContent>

              {!isLast && (
                <div
                  className='bg-border pointer-events-none absolute top-[26px] bottom-0 left-[19.5px] w-px'
                  style={{ zIndex: 0 }}
                />
              )}
            </AccordionItem>
