...
export const StepsBlock: FC<StepsBlockProps> = ({ blocks, toolResultsMap, conversationId }) => {
  ...
  const renderStepContent = (block: StepBlock) => {
    ...
    if (block.type === BLOCK_TYPE.TOOL_USE) {
      ...
      const displayContent = safeJsonParse<{ tool_name?: string }>(payload?.display_content?.json_block);
      const resolvedToolName = payload?.name || displayContent?.tool_name;

      return (
        <div className='space-y-4'>
          <BrowserLiveWatchSection conversationId={conversationId} toolName={resolvedToolName} />
        ...
        {blocks.map((block, index) => {
          ...
          return (
            <AccordionItem key={`step-${index}`} value={`step-${index}`} className='relative border-none'>
              ...
              <AccordionContent className='bg-GRAY_50 max-h-[min(90vh,720px)] overflow-y-auto py-2 pr-2 pl-11 [scrollbar-width:thin]'>
