...
>(({ className, inset, children, ...props }, ref) => (
  <DropdownMenuPrimitive.SubTrigger
    ref={ref}
    className={cn(
      'f-14-500 outline-hidden focus:bg-accent data-[state=open]:bg-accent flex cursor-default select-none items-center rounded-sm px-2 py-1.5',
      inset && 'pl-8',
      className,
    )}
    {...props}
...
>(({ className, inset, ...props }, ref) => (
  <DropdownMenuPrimitive.Item
    ref={ref}
    className={cn(
      'outline-hidden text-GRAY_900 f-12-500 font-inter flex cursor-pointer items-center gap-[6px] gap-x-4 self-stretch px-[10px] py-[8px]',
      inset && 'pl-8',
      className,
    )}
    {...props}
...
>(({ className, children, checked, onSelect, ...props }, ref) => (
  <DropdownMenuPrimitive.CheckboxItem
    ref={ref}
    className={cn(
      'f-12-500 outline-hidden focus:text-accent-foreground data-disabled:pointer-events-none data-disabled:opacity-50 pointer-events-auto relative flex cursor-pointer select-none items-center justify-between gap-x-2 rounded-sm px-2 py-1.5 transition-colors hover:bg-gray-50 focus:bg-gray-50 data-[state=open]:bg-gray-50',
      className,
    )}
    checked={checked}
    onSelect={(event) => {
...
>(({ className, inset, ...props }, ref) => (
  <DropdownMenuPrimitive.Label
    ref={ref}
    className={cn('f-14-500 px-2 py-1.5', inset && 'pl-8', className)}
    {...props}
  />
));
